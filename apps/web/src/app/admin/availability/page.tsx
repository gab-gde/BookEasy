'use client';

import { useState, useEffect } from 'react';
import { format } from 'date-fns';
import { fr } from 'date-fns/locale';
import { Plus, Pencil, Trash2, Clock, Calendar, XCircle } from 'lucide-react';
import { AvailabilityRule, AvailabilityException } from '@bookeasy/shared';
import { api } from '@/lib/api';
import { useAuthStore } from '@/lib/auth';
import { getDayName, cn } from '@/lib/utils';
import { Button, Input, Modal, EmptyState, Badge, Skeleton } from '@/components/ui';
import toast from 'react-hot-toast';

export default function AvailabilityPage() {
  const { token } = useAuthStore();
  const [rules, setRules] = useState<AvailabilityRule[]>([]);
  const [exceptions, setExceptions] = useState<AvailabilityException[]>([]);
  const [isLoading, setIsLoading] = useState(true);
  const [isRuleModalOpen, setIsRuleModalOpen] = useState(false);
  const [isExceptionModalOpen, setIsExceptionModalOpen] = useState(false);
  const [editingRule, setEditingRule] = useState<AvailabilityRule | null>(null);
  const [ruleForm, setRuleForm] = useState({ dayOfWeek: 0, startTime: '09:00', endTime: '18:00', slotStepMin: 30, capacity: 1 });
  const [exceptionForm, setExceptionForm] = useState({ date: '', isClosed: true, customStartTime: '', customEndTime: '' });

  const fetchData = async () => {
    if (!token) return;
    try {
      const [rulesData, exceptionsData] = await Promise.all([
        api.get<AvailabilityRule[]>('/admin/availability-rules', { token }),
        api.get<AvailabilityException[]>('/admin/availability-exceptions', { token }),
      ]);
      setRules(rulesData.sort((a, b) => a.dayOfWeek - b.dayOfWeek));
      setExceptions(exceptionsData);
    } catch (err) {
      toast.error('Erreur lors du chargement');
    } finally {
      setIsLoading(false);
    }
  };

  useEffect(() => { fetchData(); }, [token]);

  const openCreateRule = () => {
    setEditingRule(null);
    const usedDays = rules.map((r) => r.dayOfWeek);
    const nextDay = [0, 1, 2, 3, 4, 5, 6].find((d) => !usedDays.includes(d)) ?? 0;
    setRuleForm({ dayOfWeek: nextDay, startTime: '09:00', endTime: '18:00', slotStepMin: 30, capacity: 1 });
    setIsRuleModalOpen(true);
  };

  const openEditRule = (rule: AvailabilityRule) => {
    setEditingRule(rule);
    setRuleForm({
      dayOfWeek: rule.dayOfWeek,
      startTime: rule.startTime,
      endTime: rule.endTime,
      slotStepMin: rule.slotStepMin,
      capacity: rule.capacity,
    });
    setIsRuleModalOpen(true);
  };

  const handleRuleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!token) return;
    try {
      if (editingRule) {
        await api.patch(`/admin/availability-rules/${editingRule.id}`, ruleForm, { token });
        toast.success('Règle mise à jour');
      } else {
        await api.post('/admin/availability-rules', ruleForm, { token });
        toast.success('Règle créée');
      }
      setIsRuleModalOpen(false);
      fetchData();
    } catch (err: any) {
      toast.error(err.message || 'Erreur');
    }
  };

  const handleDeleteRule = async (id: string) => {
    if (!token) return;
    if (!confirm('Supprimer cette règle ?')) return;
    try {
      await api.delete(`/admin/availability-rules/${id}`, { token });
      toast.success('Règle supprimée');
      fetchData();
    } catch (err) {
      toast.error('Erreur');
    }
  };

  const openCreateException = () => {
    setExceptionForm({ date: '', isClosed: true, customStartTime: '', customEndTime: '' });
    setIsExceptionModalOpen(true);
  };

  const handleExceptionSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!token) return;
    try {
      await api.post('/admin/availability-exceptions', exceptionForm, { token });
      toast.success('Exception créée');
      setIsExceptionModalOpen(false);
      fetchData();
    } catch (err: any) {
      toast.error(err.message || 'Erreur');
    }
  };

  const handleDeleteException = async (id: string) => {
    if (!token) return;
    try {
      await api.delete(`/admin/availability-exceptions/${id}`, { token });
      toast.success('Exception supprimée');
      fetchData();
    } catch (err) {
      toast.error('Erreur');
    }
  };

  const dayOptions = [0, 1, 2, 3, 4, 5, 6].map((d) => ({ value: String(d), label: getDayName(d) }));

  if (isLoading) {
    return (
      <div className="space-y-8">
        <div className="card p-6"><Skeleton className="h-48 w-full" /></div>
        <div className="card p-6"><Skeleton className="h-32 w-full" /></div>
      </div>
    );
  }

  return (
    <div className="space-y-8">
      {/* Weekly rules */}
      <div className="card">
        <div className="flex items-center justify-between p-6 border-b border-slate-200">
          <div>
            <h2 className="text-lg font-semibold text-slate-900">Horaires hebdomadaires</h2>
            <p className="text-sm text-slate-500">Définissez vos jours et heures de travail</p>
          </div>
          <Button onClick={openCreateRule} disabled={rules.length >= 7}>
            <Plus className="w-4 h-4" /> Ajouter un jour
          </Button>
        </div>
        
        {rules.length === 0 ? (
          <EmptyState
            icon={Clock}
            title="Aucun horaire défini"
            description="Ajoutez vos jours de travail pour recevoir des réservations"
          />
        ) : (
          <div className="divide-y divide-slate-200">
            {rules.map((rule) => (
              <div key={rule.id} className="flex items-center justify-between p-4 hover:bg-slate-50">
                <div className="flex items-center gap-4">
                  <div className="w-24 font-medium text-slate-900">{getDayName(rule.dayOfWeek)}</div>
                  <Badge variant="info">{rule.startTime} - {rule.endTime}</Badge>
                  <span className="text-sm text-slate-500">
                    Créneaux de {rule.slotStepMin} min • {rule.capacity} place{rule.capacity > 1 ? 's' : ''}
                  </span>
                </div>
                <div className="flex gap-2">
                  <Button variant="ghost" size="sm" onClick={() => openEditRule(rule)}>
                    <Pencil className="w-4 h-4" />
                  </Button>
                  <Button variant="ghost" size="sm" onClick={() => handleDeleteRule(rule.id)}>
                    <Trash2 className="w-4 h-4 text-red-500" />
                  </Button>
                </div>
              </div>
            ))}
          </div>
        )}
      </div>

      {/* Exceptions */}
      <div className="card">
        <div className="flex items-center justify-between p-6 border-b border-slate-200">
          <div>
            <h2 className="text-lg font-semibold text-slate-900">Exceptions</h2>
            <p className="text-sm text-slate-500">Jours fériés, congés ou horaires modifiés</p>
          </div>
          <Button onClick={openCreateException}>
            <Plus className="w-4 h-4" /> Ajouter une exception
          </Button>
        </div>

        {exceptions.length === 0 ? (
          <div className="p-8 text-center text-slate-500">Aucune exception définie</div>
        ) : (
          <div className="divide-y divide-slate-200">
            {exceptions.map((exc) => (
              <div key={exc.id} className="flex items-center justify-between p-4 hover:bg-slate-50">
                <div className="flex items-center gap-4">
                  <Calendar className="w-5 h-5 text-slate-400" />
                  <span className="font-medium text-slate-900">
                    {format(new Date(exc.date), 'EEEE d MMMM yyyy', { locale: fr })}
                  </span>
                  {exc.isClosed ? (
                    <Badge variant="danger">Fermé</Badge>
                  ) : (
                    <Badge variant="warning">{exc.customStartTime} - {exc.customEndTime}</Badge>
                  )}
                </div>
                <Button variant="ghost" size="sm" onClick={() => handleDeleteException(exc.id)}>
                  <XCircle className="w-4 h-4 text-red-500" />
                </Button>
              </div>
            ))}
          </div>
        )}
      </div>

      {/* Rule Modal */}
      <Modal isOpen={isRuleModalOpen} onClose={() => setIsRuleModalOpen(false)} title={editingRule ? 'Modifier les horaires' : 'Ajouter un jour'}>
        <form onSubmit={handleRuleSubmit} className="space-y-4">
          <div>
            <label className="label">Jour de la semaine</label>
            <select
              className="input"
              value={ruleForm.dayOfWeek}
              onChange={(e) => setRuleForm({ ...ruleForm, dayOfWeek: parseInt(e.target.value) })}
              disabled={!!editingRule}
            >
              {dayOptions.map((opt) => (
                <option key={opt.value} value={opt.value}>{opt.label}</option>
              ))}
            </select>
          </div>
          <div className="grid grid-cols-2 gap-4">
            <Input
              label="Heure de début"
              type="time"
              value={ruleForm.startTime}
              onChange={(e) => setRuleForm({ ...ruleForm, startTime: e.target.value })}
              required
            />
            <Input
              label="Heure de fin"
              type="time"
              value={ruleForm.endTime}
              onChange={(e) => setRuleForm({ ...ruleForm, endTime: e.target.value })}
              required
            />
          </div>
          <div className="grid grid-cols-2 gap-4">
            <Input
              label="Durée des créneaux (min)"
              type="number"
              min={5}
              max={120}
              value={ruleForm.slotStepMin}
              onChange={(e) => setRuleForm({ ...ruleForm, slotStepMin: parseInt(e.target.value) })}
            />
            <Input
              label="Capacité par créneau"
              type="number"
              min={1}
              max={100}
              value={ruleForm.capacity}
              onChange={(e) => setRuleForm({ ...ruleForm, capacity: parseInt(e.target.value) })}
            />
          </div>
          <div className="flex justify-end gap-3 pt-4">
            <Button type="button" variant="secondary" onClick={() => setIsRuleModalOpen(false)}>Annuler</Button>
            <Button type="submit">{editingRule ? 'Enregistrer' : 'Créer'}</Button>
          </div>
        </form>
      </Modal>

      {/* Exception Modal */}
      <Modal isOpen={isExceptionModalOpen} onClose={() => setIsExceptionModalOpen(false)} title="Ajouter une exception">
        <form onSubmit={handleExceptionSubmit} className="space-y-4">
          <Input
            label="Date"
            type="date"
            value={exceptionForm.date}
            onChange={(e) => setExceptionForm({ ...exceptionForm, date: e.target.value })}
            required
          />
          <label className="flex items-center gap-2 cursor-pointer">
            <input
              type="checkbox"
              checked={exceptionForm.isClosed}
              onChange={(e) => setExceptionForm({ ...exceptionForm, isClosed: e.target.checked })}
              className="w-4 h-4 rounded border-slate-300 text-primary-600"
            />
            <span className="text-sm text-slate-700">Jour fermé (aucune réservation possible)</span>
          </label>
          {!exceptionForm.isClosed && (
            <div className="grid grid-cols-2 gap-4">
              <Input
                label="Heure de début"
                type="time"
                value={exceptionForm.customStartTime}
                onChange={(e) => setExceptionForm({ ...exceptionForm, customStartTime: e.target.value })}
              />
              <Input
                label="Heure de fin"
                type="time"
                value={exceptionForm.customEndTime}
                onChange={(e) => setExceptionForm({ ...exceptionForm, customEndTime: e.target.value })}
              />
            </div>
          )}
          <div className="flex justify-end gap-3 pt-4">
            <Button type="button" variant="secondary" onClick={() => setIsExceptionModalOpen(false)}>Annuler</Button>
            <Button type="submit">Créer</Button>
          </div>
        </form>
      </Modal>
    </div>
  );
}
